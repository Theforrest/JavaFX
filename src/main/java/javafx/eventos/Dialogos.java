...
public class Dialogos extends Application {
	...
	private RadioButton rbInformacionSinCabecera;
	private RadioButton rbAdvertencia;
	private RadioButton rbError;
	private RadioButton rbConfirmacion;
	private RadioButton rbConfirmacionPersonalizado;
	private RadioButton rbEntradaTexto;
	private RadioButton rbEleccion;
	...
	private void mostrarDialogoEntradaTexto() {
		TextInputDialog dialogo = new TextInputDialog("Patricio");
	...
	private void mostrarDialogoPersonalizado() {
		...
		tfPoder.textProperty().addListener((observable, oldValue, newValue) -> compruebaNumero(newValue, oldValue));
		CheckBox cbSuperpoder = new CheckBox();
		ChoiceBox<Estrategia> cbEstrategia = new ChoiceBox<>(FXCollections.observableArrayList(Estrategia.values()));
		...
		tfNombre.textProperty().addListener((ov, oldValue, newValue) -> btAnadir.setDisable(newValue.trim().isEmpty()));
		...
		dialogo.setResultConverter(botonPulsado -> {
		    if (botonPulsado == bttAnadir) {
		    	...
		    	return new Personaje(tfNombre.getText(), Integer.parseInt(poder), cbSuperpoder.isSelected(), cbEstrategia.getValue());
		    }
		    return null;
		});
